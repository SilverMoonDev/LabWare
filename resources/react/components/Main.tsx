...
function Main() {
  ...
  const [color, setColor] = useState(false); // State to manage the color of rows in the table
  const [sortedList, setSortedList] = useState({}); // State to determine the sorting criteria
  const [sorted, setSorted] = useState(false); // State to indicate if the list is sorted
  const [filteredList, setFilteredList] = useState([]); // State to hold the criteria for filtering the list
  const initialPosition = useRef([]); // Ref to store the initial position of the productList for reset
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      initialPosition.current = products;
    };
    fetchProducts();
  }, []);
